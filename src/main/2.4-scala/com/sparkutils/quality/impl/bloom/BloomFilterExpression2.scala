...
object BloomFilterLookupSparkVersionSpecific {
   ...
   *
   * @param expression a single Expression, can be a complex tree or simple direct probabilityIn
  ...
  def getBlooms(expression: Expression): Seq[String] = {
    ...
    def bloom(expression: Expression, acc: Seq[String]): Seq[String] = {
      ...
      // case one
      expression match {
        case UnresolvedFunction(FunctionIdentifier(name , _), Seq(left, Literal(id: UTF8String, StringType)), _) if name.replaceAll("_","").toLowerCase == "probabilityin" => bloom(left, acc :+ id.toString)
