 ...
 */
case class ExpressionLookup(attributesUsed: util.VariablesLookup.Identifiers = Set.empty, unknownSparkFunctions: util.VariablesLookup.Identifiers = Set.empty, lambdas: Set[Id] = Set.empty, sparkFunctions: Set[String] = Set.empty)
...
object VariablesLookup {
   ...
   * @return
   */
  def fieldsFromExpression(expr: Expression, knownLambdaLookups: ProcessedLambdas = Map.empty): util.ExpressionLookup = {
