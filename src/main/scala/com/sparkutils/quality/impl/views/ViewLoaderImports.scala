...
trait ViewLoading {
  ...
  def loadViewConfigs(loader: DataFrameLoader, viewDF: DataFrame,
                      ...
                      filter: Column,
                      sql: Column
                     ): (Seq[ViewConfig], Set[String]) = ViewLoader.loadViewConfigs(loader, viewDF, ruleSuiteIdColumn, ruleSuiteVersionColumn,
    ruleSuiteId, name, token, filter, sql)
  ...
  def loadViewConfigs(loader: DataFrameLoader, viewDF: DataFrame,
                      ...
                      filter: Column,
                      sql: Column
                     ): (Seq[ViewConfig], Set[String]) = ViewLoader.loadViewConfigs(loader, viewDF, name, token, filter, sql)
