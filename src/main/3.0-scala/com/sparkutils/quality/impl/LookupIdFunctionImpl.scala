  ...
  def identifyLookups(exp: Expression): Option[ExpressionLookupResult] = {
    ...
    def accumulate(res: Option[ExpressionLookupResult], exp: Expression): Option[ExpressionLookupResult] =
      exp match {
        ...
        case UnresolvedFunction(FunctionIdentifier(funcName, None), Seq(Literal(name: UTF8String, StringType), next), _, _) if funcName.replaceAll("_","").toLowerCase == "maplookup" || funcName.replaceAll("_","").toLowerCase =="mapcontains" =>
        ...
        case UnresolvedFunction(FunctionIdentifier(funcName, None), Seq(exp, next), _, _) if funcName.replaceAll("_","").toLowerCase == "maplookup" || funcName.replaceAll("_","").toLowerCase =="mapcontains" =>
          accumulate(res.map(r => r.copy(hasExpressionLookups = true)).orElse(Some(ExpressionLookupResult(Set.empty, true))), next)

        // unresolved case where we cannot see more unresolved functions
        case UnresolvedFunction(FunctionIdentifier(funcName, None), Seq(next, Literal(name: UTF8String, StringType)), _, _) if funcName.replaceAll("_","").toLowerCase == "probabilityin" =>
        ...
        // unresolved case where we have constants - it'll fail  at creation
        case UnresolvedFunction(FunctionIdentifier(funcName, None), Seq(next, exp), _, _) if funcName.replaceAll("_","").toLowerCase == "probabilityin" =>
          accumulate(res.map(r => r.copy(hasExpressionLookups = true)).orElse(Some(ExpressionLookupResult(Set.empty, true))), next)
