...
def guess_filename(packagename, filename):
    combined = packagename + '/' + filename
    #CTw - attempt prefixes until a match is found, java and scala only atm
    j = "src/main/java/" + combined
    s = "src/main/scala/" + combined
    return (s if os.path.exists(s) else j)
...
def convert_class(j_class, j_package):
    ...
    # CTw - change to use package name + class sourcefilename 
    c_class.set('filename', guess_filename(j_package.attrib['name'], j_class.attrib['sourcefilename']))
