...
import com.sparkutils.quality.{DataFrameLoader, Id, ViewConfigColumns}
...
trait ViewLoading {
   ...
   */
  def loadViewConfigs(loader: DataFrameLoader, viewDF: DataFrame,
                      ruleSuiteId: Id, viewColumns: ViewConfigColumns = ViewConfigColumns()): (Seq[ViewConfig], Set[String]) =
    ConfigLoader.loadConfigs[ViewConfig, ViewRow](
      loader, viewDF,
      viewColumns.ruleSuiteId,
      viewColumns.ruleSuiteVersion,
      ruleSuiteId,
      viewColumns.name,
      viewColumns.token,
      viewColumns.filter,
